...
void tb600b_get_combined_data(uart_port_t uart_num, const uint8_t *command, size_t commandSize, const char *tag,
  ...
  // Initial check
  // Optional but recommended: Check if any of the output pointers are NULL.
  ...
  if (bytesRead == responseLength) {
    ...
    if (responseData[0] == 0xFF && responseData[1] == 0x87) {
      ...
      // *** The Key Modification: Store the results using the dereference operator '*' ***
      *out_temperature = temperature;
      *out_humidity = humidity;
      *out_gasUg = gasUg;

      // Show log
      // ESP_LOGI(tag, "Temperature: %.2f °C", *out_temperature);
      // ESP_LOGI(tag, "Humidity: %.2f %%", *out_humidity);
      // ESP_LOGI(tag, "Gas Concentration: %.2f ug/m^3", *out_gasUg);
    } else {
